/* ============================
   script.js — Advanced TRX Miner
   - Persistent state via localStorage
   - Mining animation + progress (resume after refresh)
   - Convert TRX→USDT (rate = 0.0006)
   - Withdraw requests (min 0.01 USDT)
   - VIP buy + proof upload (stored as base64 locally)
   - Telegram notify helper (place token & chat id)
   ============================ */

/* ========= CONFIG ========= */
// Put your real bot token & admin chat id here when you want real Telegram messages.
// If TELEGRAM_BOT_TOKEN is empty, messages won't be sent (safe).
const TELEGRAM_BOT_TOKEN = "";      // <- paste your token here later (e.g. "7659505:AA...") or leave blank
const ADMIN_CHAT_ID = "7417215529"; // admin/chat id

/* ======= CONSTANTS ======= */
const STORAGE_KEY = "trx_advanced_v2";
...
navBtns.forEach(btn=>{
  btn.addEventListener('click', ()=>{
    ...
    const elId = target.startsWith('page-') ? target : `page-${target}`;
    const el = document.getElementById(elId) || document.getElementById(target);
...
/* toggle coin spin animation by adding class to .coin-outer */
...
submitWithdraw && submitWithdraw.addEventListener('click', ()=>{
  ...
  if(isNaN(amount) || amount < 0.01){ alert('Amount must be >= 0.01 USDT'); return; }
...
/* already handled above — copy functions */
...
function sendTelegramMessage(text){
  ...
  }).then(r=>r.json()).then(res=> console.log('tg ok', res)).catch(e=> console.error('tg err', e));
}

/* ======= CSS runtime: add spin class CSS ======= */
(function addSpinCSS(){
